<form action="#">
    <!--item Food box-->
    <div class="bn-take-away-item bn-catering-drink bn-optionals-item">
        <div class="container">
            <div class="bn-drink-heading">
                <p><b>{{ __('catering.add_sides_drinks') }} </b> {{ __('catering.no_minimum_quantity') }}</p>
            </div>
            <div class="row">
                <div class="col-lg-3">
                    <div class="bn-left-side-bar sticky-sm-top">
                        ...
                        <button type="button" class="btn bg-dark w-100 text-white" onclick="window.location.href='{{ route('catering.checkout') }}'">
                            {{ __('global.continue') }}
                        </button>
                    </div>
                </div>
                <div class="col-lg-9 bn-right-side-bar-product">
                    <div class="bn-toggle-content">
                        @foreach($items as $item)
                            <div class="row">
                                <div class="col-md-4">
                                    <img class="bn-thumbnail-img"
                                         src="{{ $item->image_thumbnail }}" alt="">
                                </div>
                                ...
                                <div class="col-md-3 bn-bg-product">
                                    ...
                                    <div class="bn-number-product">
                                        <img src="{{ asset('asstes/image/take-away/min.svg') }}" alt=""
                                             data-add="-1" data-code="{{ $item->code }}" class="addSide">
                                        <span>x{{ $session['portion']['sides'][$item->code] ?? 0 }}</span>
                                        <img src="{{ asset('asstes/image/take-away/max.svg') }}" alt="" data-add="1"
                                             data-code="{{ $item->code }}" class="addSide">
                                    </div>
